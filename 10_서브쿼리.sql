...
-- 다음 문장을 서브쿼리가 리턴하는 행이 여러 개라서 단일행 연산자(> >= < <= = <>)를 사용할 수 없습니다
-- 단일 행 연산자: 주로 비교 연산자 (=, >, <, <=, >= <>)를 사용하는 경우 하나의 행만 반환해야 합니다.
-- 이런 경우에는 다중 행 연산자를 사용해야 합니다.
SELECT * FROM employees
WHERE job_id = (SELECT job_id FROM employees
                WHERE last_name = 'King'); -- The sub will make the code an error because the return value has two values. Which is impossible to print both. 
-- For a subquery to work the return value has to be one. When the return value has multiple value then it will result into an error. When using (=, >, <, <=, >= <>)
...
-- first_name이 Dave인 사람들 급여와 같은 급여를 받는 사람들을 조회.
SELECT * FROM employees
WHERE salary IN (SELECT salary FROM employees
                WHERE first_name = 'David');
                
-- ANY, SOME(Oracle 적용): 값을 서브쿼리에 의해 리턴된 각각의 값과 비교해서
-- 하나라도 만족하면 조회 대상에 포함됩니다.
-- David라는 사람이 여러 명인데, 그 중에 적은 급여를 받는 David보다 급여가 높은 사람을 조회.
SELECT * FROM employees
...
-- EXISTS 연산자가 1이 조회가 될 때 데이터가 존재한다는 것을 판단하여 employees에서 해당 사원의 정보를 조회.
