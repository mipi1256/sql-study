...
-- 시퀀스(SEQUENCE)(순차적으로 증가하는 값을 만들어 주는 객체)
...
CREATE SEQUENCE dept2_seq 
    ...
    MINVALUE 1 -- 최소값 (기본값은 증가 일때 1, 감소일 때 -1028)
    NOCACHE -- 캐시 메모리 상요 여부 (CACHE)
...
ALTER SEQUENCE dept2_seq MINVALUE 0; -- 최소값 

-- 시퀀스 값을 다시 처음으로 돌리는 방법 
-- This is a way to reset the SEQUENCE but it more work than usual.
-- So its better to drop the SEQUENCE and then make a new SEQUENCE.

ALTER SEQUENCE dept2_seq INCREMENT BY -10;
SELECT dept2_seq.NEXTVAL FROM dual;
ALTER SEQUENCE dept2_seq INCREMENT BY 1;

-- If you have the SEQUENCE code that was used like written in a code
-- Then its better to DROP the SEQUENCE and make the new SEQUENCE
