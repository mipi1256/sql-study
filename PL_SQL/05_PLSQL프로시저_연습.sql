...
BEGIN
    ...
    ELSIF p_flag = 'D' THEN
        ...
        WHERE department_id = p_dept_id;
    ELSE
        dbms_output.put_line('해당 flag에 대한 동작이 존비되지 않았습니다.');
    END IF;
...
EXEC depts_proc(80, '영업부', 'I'); -- I: INSERT, U: UPDATE, D: DELETE
...
-- MY CODE 
-- MISSING EXCEPTION AND SOME PARTS ARE MISSING. 
CREATE PROCEDURE depts_proc
    (p_depart_id IN depts.department_id%TYPE,
    p_depart_name IN depts.department_name%TYPE,
    p_flag IN VARCHAR2)
IS
BEGIN
    ...
    IF p_flag = 'I' THEN
        INSERT INTO depts
        VALUES(p_depart_id, p_depart_name);
    ELSE IF p_flag = 'U' THEN
        UPDATE INTO depts
        SET depts.department_name = p_depart_name
        WHERE depts.department_id = p_depart_id;
    ELSE IF p_flag = 'D' THEN
        DELETE FROM depts 
        WHERE depts.department_id = p_depart_id;
    ELSE 
        dbms_output.put_line('I: INSERT, U: UPDATE, D: DELETE');
    END IF;
