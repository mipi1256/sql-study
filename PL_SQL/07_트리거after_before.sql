...
BEGIN
    ...
    -- INSERT 예정인 name의 값에서 첫글자만 추출 후 뒤에 *을 세개 붙이겠다.
    -- On the next name the first letter will be called and on the rest will be shown as ***
...
DECLARE
    ...
    quantity_shortage_exception EXCEPTION; -- New exception made by me
    zero_total_exception EXCEPTION;
BEGIN
    dbms_output.put_line('트기러 실행!');
    ...
    FROM product
    WHERE product_no = v_product_no; -- 상품 번호를 가지고 제고 수량을 조회 -> v_product_total에 할당.
    
    IF v_product_total <= 0 THEN -- 재고가 없는 경우 zero_total_exception
        RAISE zero_total_exception;
    ELSIF v_total > v_product_total THEN -- 주문수량이 재고수량보다 많은 경우 quantity_shortage_exception
    ...
    EXCEPTION
        WHEN quantity_shortage_exception THEN
            ...
            -- 두번째 매개값: 에러 메세지
            RAISE_APPLICATION_ERROR(-20001, '주문하신 수량보다 재고가 적어서 주물할 수 없습니다.');
        WHEN zero_total_exception THEN
            RAISE_APPLICATION_ERROR(-20002, '주문사진 상품의 재고가 없어 주문할 수 없습니다');
...
-- 트기러 내에서 예외가 발생하면 수행중인 INSERT 작업은 중단되며 ROLLBACK이 진행됩니다.
