...
-- 매개값(인수) 없는 프로시저
CREATE PROCEDURE p_test -- 프로시저 만든 방법
IS -- 선언부
...
CREATE OR REPLACE PROCEDURE my_new_job_proc
    (p_job_id IN jobs.job_id%TYPE,
     p_job_title IN jobs.job_title%TYPE,
     p_min_sal IN jobs.min_salary%TYPE := 0, --This will make the value the default value
     p_max_sal IN jobs.max_salary%TYPE := 1000
    )
IS
    v_cnt NUMBER := 0;
BEGIN
    -- 동일한 job_id가 있는지 부터 체크
    -- 이미 존재한다면 1, 존재하지 않는다면 0 -> v_cnt에 저장
    SELECT
        COUNT(*)
    INTO
        v_cnt -- 1 or 0 (1 means it exist / 0 means it doesn't)
    FROM jobs
...
EXEC my_new_job_proc('JOB3', 'test job3', 8000, 10000); -- New data will be added
SELECT * FROM jobs;
EXEC my_new_job_proc('JOB2', '테스트 job2', 10000, 120000); -- JOB2 already exists so it will only update the data that needs to be updated.

EXEC my_new_job_proc('JOB5', 'test job5'); -- This can be use when the other variables already has a default value
...
CREATE OR REPLACE PROCEDURE my_new_job_proc
    (p_job_id IN jobs.job_id%TYPE,
     p_job_title IN jobs.job_title%TYPE,
     p_min_sal IN jobs.min_salary%TYPE := 0, --This will make the value the default value
     p_max_sal IN jobs.max_salary%TYPE := 1000, -- This will make this value the default value
...
BEGIN
    ...
    my_new_job_proc('CEO1', 'test ceo1', 30000, 80000, msg);
    dbms_output.put_line(msg);
END;
...
-- RETURN: 값의 반환 X, 프로시저 강제 종료 할 때 사용.
...
BEGIN
    ...
    */
    ...
    EXCEPTION
        ...
        WHEN OTHERS THEN -- JAVA(Catch(Exception E){})
