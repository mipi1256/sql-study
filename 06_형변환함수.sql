...
-- 날짜 문자로 형 변환할 때 원하는 형식으로 날짜를 표현할 수 있습니다. 
...
SELECT 
    TO_CHAR(sysdate, 'YYYY"년"-MM"월"-DD"일"') -- To write down multiple information use "" to add them
FROM dual;
...
SELECT TO_CHAR(20000.29, '99999.9') FROM dual; -- 반올림 지원. Work also for rounding up the number.
...
SELECT '2000' + 2000 FROM dual; -- 자동 형 변화 (문자 -> 숫자)
...
SELECT TO_DATE('2021년 02월 12일', 'YYYY"년"MM"월"DD"일"') FROM dual;

-- xxxx년 xx월 xx일 -> 문자열 형식으로 변환 해보세요.
...
-- NULL 형태를 변환하는 함수 NVL2(컬럼, NULL이 아닌 경우의 값, NULL일 경우의 값)
SELECT 
    first_name,
    salary,
    NVL2(
